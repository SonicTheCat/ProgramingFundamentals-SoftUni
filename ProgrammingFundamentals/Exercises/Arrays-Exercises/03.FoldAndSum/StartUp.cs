namespace practice
{
    ...
    public class StartUp
    {
        public static void Main()
        {
            ...
            int[] leftSide = new int[input.Length / 4];
            int[] rightSide = new int[input.Length / 4];
            int[] middlePart = new int[input.Length / 2];

            FillLeftSideOfArray(leftSide, input);
            FillRightSideOfArray(rightSide, input);
            FillMiddlePartOfArray(input, middlePart, leftSide);

            int[] bothLeftAndRightSide = new int[input.Length / 2];
